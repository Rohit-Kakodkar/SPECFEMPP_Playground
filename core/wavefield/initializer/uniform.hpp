...
class WavefieldUniformInitializer2D {
public:
    WavefieldUniformInitializer2D(const size_t nelements, const size_t nz, const size_t nx,
    ...
    void initialize(ViewType& view) const {
        view = ViewType("UniformInitializedView", nelements_, nz_, nx_, ncomponents_, ncomponents_);
        using ExecSpace = typename ViewType::execution_space;
        Kokkos::parallel_for(
            "InitializeUniformView",
